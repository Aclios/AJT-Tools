...
import mmh3
from .REWString import decrypt, wcharPool2StrDict, seekString, calcStrPoolOffsets, toWcharBytes, encrypt
...
class MSG:
    ...
    def readMSG(self, filestream: io.BufferedReader):
        ...
        if isVersionEncrypt(version):
            wcharPool = decrypt(data)
        else:
            wcharPool = data
        stringDict = wcharPool2StrDict(wcharPool)

        # read attribute name to attributeHeaders
        for i, attrHead in enumerate(attributeHeaders):
            attrHead["name"] = seekString((attributeNamesOffsets[i] - dataOffset), stringDict)

        # get content of each entry
        for entryIndex, entry in enumerate(entrys):
            # set entry name
            entry.setName(seekString((entry.entryNameOffset - dataOffset), stringDict))
            ...
            for strOffset in entry.contentOffsetsByLangs:
                lang.append(seekString((strOffset - dataOffset), stringDict))
            entry.setContent(lang)

            # seek string value of each attribute
            for i, attrHead in enumerate(attributeHeaders):
                if attrHead["valueType"] == 2:
                    entry.attributes[i] = seekString((entry.attributes[i] - dataOffset), stringDict)
                elif attrHead["valueType"] == -1:
                    temp = seekString((entry.attributes[i] - dataOffset), stringDict)
    ...
    def writeMSG(self) -> bytes:
        ...
        strOffsetDict = calcStrPoolOffsets(stringPoolSet)  # not doing string processing here, as it will change the key.
        # debug use, to let input output stringpool keeps same
        # strOffsetDict = dict((v,k) for k,v in self.stringDict.items())
        wcharPool = b"".join(toWcharBytes(x) for x in strOffsetDict.keys())

        if isVersionEncrypt(self.version):
            newFile.extend(encrypt(wcharPool))
        else:
