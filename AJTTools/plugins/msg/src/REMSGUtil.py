...
import chardet
import mmh3
from .REMSG import MSG, Entry, LANG_LIST, VERSION_2_LANG_COUNT, isVersionEntryByHash, MHR_SUPPORTED_LANG, isVersionIgnoreUnusedLang
from .REWString import forceWindowsLineBreak
...
def importCSV(msgObj: MSG, filename: str, version: int = None, langCount: int = None) -> MSG:
    ...
    if langCount is None:
        if msg is not None:
            ...
            langCount = VERSION_2_LANG_COUNT[version]
    ...
    for i, fEntry in enumerate(fEntrys):
     ...
     #   assert len(contents) == langCount, f"Invalid number of language / contents.\n{"\n".join(contents)}"
        entry.buildEntry(
            ...
            langs=[forceWindowsLineBreak(content) for content in contents],
            hash=mmh3.hash(key=fEntry[nameidx].encode("utf-16-le"), seed=0xFFFFFFFF, signed=False) if isVersionEntryByHash(version) else None,
...
def exportTXT(msg: MSG, filename: str, langIndex: int, encode=None):
...
def importTXT(msgObj: MSG, filename: str, langIndex: int, encode=None) -> MSG:
...
def importJson(msgObj: MSG, filename: str):
    ...
    for jIndex, jEntry in enumerate(mhriceJson["entries"]):
        ...
        entry.buildEntry(
            ...
            langs=list([forceWindowsLineBreak(content) for content in jEntry["content"]]),
            hash=mmh3.hash(key=jEntry["name"].encode("utf-16-le"), seed=0xFFFFFFFF, signed=False) if isVersionEntryByHash(msg.version) else None,
